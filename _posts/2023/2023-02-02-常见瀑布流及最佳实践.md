...
2. 我们可以事先从后台获取到图片的宽高，因此我们就可以针对每一张图片设置和其等比例的容器。[demo](https://jsbin.com/zepofud/3/edit?html,css,output) 。实现的方式就是先设定高度为 200px ，然后让每个图片容器的宽度等于 基准值*图片宽高比，最后设置``padding-bottom``的比例为元素的高宽比。
